...
//----------------------------------------
uint8_t TS_IORead(uint8_t Addr, uint8_t Reg)
{
 uint8_t read_value = 0;
 HAL_I2C_Mem_Read(&hi2c1, Addr, (uint16_t)Reg, I2C_MEMADD_SIZE_8BIT, &read_value, 1, 0x1000);
 return read_value;
}
//----------------------------------------
void TS_IOWrite(uint8_t Addr, uint8_t Reg, uint8_t Value)
{
...
//----------------------------------------
void ft5336_TS_Get_GestureID(uint16_t DeviceAddr, uint32_t * pGestureId)

{
	volatile uint8_t ucReadData = 0;
	ucReadData = TS_IORead(DeviceAddr, FT5336_GEST_ID_REG);
					 ...
					 uint32_t * pEvent)
{
	...
	if(touchIdx < ft5336_handle.currActiveTouchNb)
	{
		...
		ucReadData = TS_IORead(DeviceAddr, regAddressXHigh);
		* pEvent = (ucReadData & FT5336_TOUCH_EVT_FLAG_MASK) >> FT5336_TOUCH_EVT_FLAG_SHIFT;
		ucReadData = TS_IORead(DeviceAddr, regAddressPWeight);
		* pWeight = (ucReadData & FT5336_TOUCH_WEIGHT_MASK) >> FT5336_TOUCH_WEIGHT_SHIFT;
		ucReadData = TS_IORead(DeviceAddr, regAddressPMisc);
...
uint16_t Touch_ReadID(uint16_t DeviceAddr)
{
 ...
 {
   ucReadId = TS_IORead(DeviceAddr, FT5336_CHIP_ID_REG);
...
uint8_t TS_DetectTouch(uint16_t DeviceAddr)
{
	...
	nbTouch = TS_IORead(DeviceAddr, FT5336_TD_STAT_REG);
...

{
 ...
 if(ft5336_handle.currActiveTouchIdx < ft5336_handle.currActiveTouchNb)
 {
	...
	ucReadData = TS_IORead(DeviceAddr, regAddressXLow);
	coord = (ucReadData & FT5336_TOUCH_POS_LSB_MASK) >> FT5336_TOUCH_POS_LSB_SHIFT;
	ucReadData = TS_IORead(DeviceAddr, regAddressXHigh);
	coord |= ((ucReadData & FT5336_TOUCH_POS_MSB_MASK) >> FT5336_TOUCH_POS_MSB_SHIFT) << 8;
	*X = coord;
	ucReadData = TS_IORead(DeviceAddr, regAddressYLow);
	coord = (ucReadData & FT5336_TOUCH_POS_LSB_MASK) >> FT5336_TOUCH_POS_LSB_SHIFT;
	ucReadData = TS_IORead(DeviceAddr, regAddressYHigh);
	coord |= ((ucReadData & FT5336_TOUCH_POS_MSB_MASK) >> FT5336_TOUCH_POS_MSB_SHIFT) << 8;
	*Y = coord;
...
uint8_t TS_Get_GestureId(TS_StateTypeDef *TS_State)
{
	...
	ft5336_TS_Get_GestureID(TS_I2C_ADDRESS,&gestureId);
	switch(gestureId)
	  {
...
void Touch_Ini(void)
{
	...
	TS_IOWrite(TS_I2C_ADDRESS, FT5336_GMODE_REG, regValue);
